...
目前共用分析契約由 `src/domain/` 定義；`src/services/` 對 UI 提供
`analyzeMaterial`、`getCachedAnalysis`、`generateSelectedImage` 與
`buildWorksheet`。服務回傳的失敗結果使用 `Result<T, AppError>`，錯誤類別包含
`validation`、`network`、`authentication` 與 `quota`。
