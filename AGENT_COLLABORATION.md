...
- 在 `src/services/` 提供明確的 use cases，例如 `analyzeMaterial`、`getCachedAnalysis`、`generateSelectedImage`、`buildWorksheet`。
...
- 將「分析」、「圖片生成」等昂貴行為設成使用者明確按鈕，不在 mount、輸入變更或自動重繪時觸發。
